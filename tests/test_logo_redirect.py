...
@allure.suite("Редиректы по логотипа")
class TestLogoRedirect:

    @allure.title("Редирект на Яндекс при клике на логотип")
    def test_logoredirect_to_yandex_success(self, main_page):
        ...
        assert ExpectedURLS.YANDEX in main_page.get_current_url()

    @allure.title("Редирект на главную страницу Самоката при клике на логотип")
    def test_logoredirect_to_scooter_main_page_success(self, main_page):
        main_page.go_to_url(URLS.BASE_URL)
        main_page.click_logo_samocat()

        assert URLS.BASE_URL == main_page.get_current_url()
